<nav x-data="{ open: false }" class="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
        ...
        <div class="flex justify-between h-14 items-center">
            <div class="flex items-center">
                ...
                <div class="hidden space-x-4 sm:-my-px sm:ms-6 sm:flex">
                    ...
                    <!-- Dashboard -->
                    <x-nav-link :href="route('dashboard')" :active="request()->routeIs('dashboard')" class="text-sm py-1">
                        📊 {{ __('Dashboard') }}
                    </x-nav-link>

                    <!-- Rekomendasi -->
                    <x-nav-link :href="route('rekomendasi')" :active="request()->routeIs('rekomendasi')" class="text-sm py-1">
                        🔍 {{ __('Rekomendasi') }}
                    </x-nav-link>

                    <!-- Manage Smartphone -->
                    <x-nav-link :href="route('smartphones.index')" :active="request()->routeIs('smartphones.*')" class="text-sm py-1">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            <!-- Menu mobile dengan text-sm -->
            <x-responsive-nav-link :href="route('beranda')" :active="request()->routeIs('beranda')" class="text-sm py-2">
                🏠 {{ __('Beranda') }}
            </x-responsive-nav-link>
            <x-responsive-nav-link :href="route('dashboard')" :active="request()->routeIs('dashboard')" class="text-sm py-2">
                📊 {{ __('Dashboard') }}
            </x-responsive-nav-link>
            <x-responsive-nav-link :href="route('rekomendasi')" :active="request()->routeIs('rekomendasi')" class="text-sm py-2">
                🔍 {{ __('Rekomendasi') }}
            </x-responsive-nav-link>
            <x-responsive-nav-link :href="route('smartphones.index')" :active="request()->routeIs('smartphones.*')" class="text-sm py-2">
                📱 {{ __('Manage Smartphone') }}
            </x-responsive-nav-link>
        </div>

        <!-- Responsive Settings Options -->
        <div class="pt-3 pb-2 border-t border-gray-200 dark:border-gray-600">
            <div class="px-4 py-2">
